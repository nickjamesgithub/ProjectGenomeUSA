...
sp_500 = True
# Define parameter to select either Genome_classification or Genome_classification_bespoke
use_bespoke_classification = True  # Set to True to use Genome_classification_bespoke, False to use Genome_classification
...
def sector_functions_mobility_matrix(df):
    ...
    for i in range(len(unique_companies)):
        ...
        features_list.append([revenue_avg, revenue_cagr, leverage, investment, ep_fe_avg, acquisition_propensity, capex_per_revenue_avg, npat_per_employee_avg, gross_margin_avg, tsr])
...
def generate_bespoke_genome_classification_df(df):
    ...
    for sector in df["Sector"].unique():
        ...
        print(f"Processing sector: {sector} | Metric used: {metric}")

        if metric not in df.columns:
            print(f"Warning: Metric '{metric}' for sector '{sector}' not found. Skipping sector.")
            continue
...
df_merge_ = generate_genome_classification_df(df_concat)
df_merge = generate_bespoke_genome_classification_df(df_merge_)
...
features = ["Company_name", "Sector", "Year", "TSR", "Revenue_growth_3_f", "Stock_Price", "Adjusted_Stock_Price","DPS", "BBPS",
            "DBBPS", "Genome_classification", "Genome_classification_bespoke", "Price_to_Book", "PE_Implied", "Market_Capitalisation",
            "Revenue", "Debt_to_equity", "RD/Revenue",
            "EP/FE", "ROE_above_Cost_of_equity", "CROTE_TE",
    ...
    for j in range(len(unique_tickers)):
        try:
            ...
            if df_slice_i["EP/FE"].values[0] < 0 and df_slice_i_2["EP/FE"].values[0] < 0:
                ...
                df_list.append(["Remain_negative", genome_classification_beginning, genome_classification_end, genome_classification_bespoke_beginning, genome_classification_bespoke_end, company_name_j, sector_j, int(year_i), int(year_i_2), df_slice_i["Revenue_growth_3_f"].values[0], df_slice_i_2["Revenue_growth_3_f"].values[0],
            ...
            if df_slice_i["EP/FE"].values[0] < 0 and df_slice_i_2["EP/FE"].values[0] >= 0:
                ...
                # Append Direction, company name, Year_0, Year_2, EP/FE_0, EP/FE_2, Stock Price_0, Stock_Price_2, DBBPS, Annualized TSR
                df_list.append(["Move_up", genome_classification_beginning, genome_classification_end, genome_classification_bespoke_beginning, genome_classification_bespoke_end, company_name_j,sector_j, int(year_i), int(year_i_2), df_slice_i["Revenue_growth_3_f"].values[0], df_slice_i_2["Revenue_growth_3_f"].values[0],
                                df_slice_i["EP/FE"].values[0],df_slice_i_2["EP/FE"].values[0],
                                df_slice_i["ROE_above_Cost_of_equity"].values[0], df_slice_i_2["ROE_above_Cost_of_equity"].values[0],
                                df_slice_i["CROTE_TE"].values[0], df_slice_i_2["CROTE_TE"].values[0],
                                df_slice_i["Stock_Price"].values[0], df_slice_i_2["Stock_Price"].values[0], df_slice_i["Price_to_Book"].values[0], dbbps_total, degrees, radians,
            ...
            if df_slice_i["EP/FE"].values[0] >= 0 and df_slice_i_2["EP/FE"].values[0] >= 0:
                ...
                # Append Direction, company name, Year_0, Year_2, EP/FE_0, EP/FE_2, Stock Price_0, Stock_Price_2, DBBPS, Annualized TSR
                df_list.append(["Remain_positive", genome_classification_beginning, genome_classification_end, genome_classification_bespoke_beginning, genome_classification_bespoke_end, company_name_j,sector_j, int(year_i), int(year_i_2), df_slice_i["Revenue_growth_3_f"].values[0], df_slice_i_2["Revenue_growth_3_f"].values[0],
                                df_slice_i["EP/FE"].values[0],df_slice_i_2["EP/FE"].values[0],
                                df_slice_i["ROE_above_Cost_of_equity"].values[0], df_slice_i_2["ROE_above_Cost_of_equity"].values[0],
                                df_slice_i["CROTE_TE"].values[0], df_slice_i_2["CROTE_TE"].values[0],
                                df_slice_i["Stock_Price"].values[0], df_slice_i_2["Stock_Price"].values[0], df_slice_i["Price_to_Book"].values[0], dbbps_total, degrees, radians,
            ...
            if df_slice_i["EP/FE"].values[0] > 0 and df_slice_i_2["EP/FE"].values[0] < 0:
                ...
                df_list.append(["Move_down",genome_classification_beginning, genome_classification_end, genome_classification_bespoke_beginning, genome_classification_bespoke_end, company_name_j, sector_j, int(year_i), int(year_i_2), df_slice_i["Revenue_growth_3_f"].values[0], df_slice_i_2["Revenue_growth_3_f"].values[0],
                                df_slice_i["EP/FE"].values[0],df_slice_i_2["EP/FE"].values[0],
                                df_slice_i["ROE_above_Cost_of_equity"].values[0], df_slice_i_2["ROE_above_Cost_of_equity"].values[0],
                                df_slice_i["CROTE_TE"].values[0], df_slice_i_2["CROTE_TE"].values[0],
                                df_slice_i["Stock_Price"].values[0], df_slice_i_2["Stock_Price"].values[0], df_slice_i["Price_to_Book"].values[0], dbbps_total, degrees, radians,
...
# Create columns
df_journey_collapsed.columns = ["Journey", "Genome_classification_beginning", "Genome_classification_end", "Genome_classification_bespoke_beginning", "Genome_classification_bespoke_end",
                                "Company_name", "Sector", "Year_beginning", "Year_final",
...
df_journey_collapsed["Y_change_standard"] = df_journey_collapsed["EP/FE_end"] - df_journey_collapsed["EP/FE_beginning"]
# Calculate Y_change_bespoke based on the bespoke metric for each sector
df_journey_collapsed["Y_change_bespoke"] = df_journey_collapsed.apply(
    lambda row: row[sector_metric_mapping.get(row["Sector"], "EP/FE") + "_end"] -
                row[sector_metric_mapping.get(row["Sector"], "EP/FE") + "_beginning"],
    axis=1
)

if genome_filtering:
    df_journey_collapsed = df_journey_collapsed.loc[(df_journey_collapsed["EP/FE_end"] >= -.3) & (df_journey_collapsed["EP/FE_end"] <= .5) &
                                                    (df_journey_collapsed["ROE_above_Cost_of_equity_end"] >= -.3) & (df_journey_collapsed["ROE_above_Cost_of_equity_end"] <= .5) &
                                                    (df_journey_collapsed["CROTE_TE_end"] >= -.3) & (df_journey_collapsed["CROTE_TE_end"] <= .5) &
...
df_journey_collapsed.to_csv(r"C:\Users\60848\OneDrive - Bain\Desktop\Project_Genome\Journeys_summary_USA.csv")
